...
// Prompt user to delete any records not found in CSVs
$deleteRecords = prompt("Do you want to delete records not found in the CSVs?");
...
importCSVData($connection, $deleteRecords);

// Get all TLDs that currently exist
function getExistingExtensions($connection)
{
    $existingExtensions = [];
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $existingExtensions[$row['id']] = $row['extension'];
        }
    }
...
// Function to prompt the user for input (Y/N)
function prompt($message)
{
    echo $message . " (Y/N): ";
    $input = trim(fgets(STDIN));
    return strtoupper($input) === 'Y';
}

function importCSVData($connection, $deleteRecords)
{
    if ($deleteRecords) {
        $existingRecords = getExistingExtensions($connection);
    }
        ...
        try {
            ...
            foreach ($csvData as $row => $data) {
                ...
                if (!$data[0] == '' || $data[0]) {
                    ...
                    if ($deleteRecords && $existingRecords) {
                        $extensionId = array_search($extension, $existingRecords);
                        if ($extensionId !== false) {
                            unset($existingRecords[$extensionId]);
                        }
                    }
                } else {
                    continue;
    ...
    if ($deleteRecords && !empty($existingRecords)) {
        // Remove any records not updated
        $extensionIdsToDelete = implode(',', array_keys($existingRecords));

        // Delete records from tbldomainpricing
        $deleteDomainPricingQuery = "DELETE FROM tbldomainpricing WHERE id IN ($extensionIdsToDelete)";
        if ($connection->query($deleteDomainPricingQuery) === TRUE) {
            echo "Successfully deleted " . mysqli_affected_rows($connection) . " TLD records not found in CSVs from tbldomainpricing." . PHP_EOL;
        } else {
            echo "Error deleting records from tbldomainpricing: " . $connection->error . PHP_EOL;
        }

        // Delete related records from tblpricing
        $deletePricingQuery = "DELETE FROM tblpricing WHERE relid IN ($extensionIdsToDelete)";
        if ($connection->query($deletePricingQuery) === TRUE
        ) {
            echo "Successfully deleted " . mysqli_affected_rows($connection) . " related pricing records not found in CSVs from tblpricing." . PHP_EOL;
        } else {
            echo "Error deleting related pricing records from tblpricing: " . $connection->error . PHP_EOL;
        }
    } elseif ($deleteRecords && !empty($existingRecords)) {
        echo "All existing domains were found in the parsed CSVs. Nothing was deleted." . PHP_EOL;
    }
